<div class="mt-8 mb-24">
    @if(0 < count($tasks) && $testTitle != "")
        ...
        <div class="fixed bottom-0 right-0">
            <button type="button"
                    class=" flex items-center py-3 focus:outline-none text-white bg-green-700 hover:bg-green-800 focus:ring-4 focus:ring-green-300 font-medium rounded-lg text-sm px-2.5 mr-2 mb-2 dark:bg-green-600 dark:hover:bg-green-700 dark:focus:ring-green-800"
    ...
    <div class="max-w-full mx-auto rounded-xl overflow-hidden  lg:w-4/6 md:w-8/12 sm:w-11/12 w-11/12">
        ...
        <div class="flex flex-col w-full relative">
            <div class="flex flex-row justify-start">
                ...
                <div class="ml-6 w-full">
                    <input type="text" id="titleOfTestAttempt" wire:model="testTitle"
                           class="w-full bg-orange-200 border-4 rounded-lg text-lg placeholder-[#716156]">
                </div>
            </div>
            <div class="flex flex-row justify-start">
                <div class="w-4/12">
            ...
            <div class="flex flex-row justify-start">
                <div class="w-4/12">
                    <p><label for="durationOfTest">Teszt kitöltési ideje </label></p>
                </div>
                <div class="ml-6 w-full flex">
                    <div>
                        <select wire:model.defer="durationHour" id="durationHour" class="w-fit">
                            <option value="null" disabled hidden>--</option>
                            @for($i = 0; $i < 10; $i++)
                                <option value="{{ $i }}" wire:key="hour-{{ $i }}">{{ $i }}</option>
                            @endfor
                        </select>
                        <label for="durationHour">óra</label>
                    </div>
                    <div>
                        <select wire:model.defer="durationMinute" id="durationMinute" class="w-fit">
                            <option value="null" disabled hidden>--</option>
                            @for($i = 0; $i < 10; $i++)
                                <option value="{{ $i }}" wire:key="minute-{{ $i }}">{{ $i }}</option>
                            @endfor
                        </select>
                        <label for="durationMinute">perc</label>
                    </div>
                </div>
            </div>
            <div class="flex flex-row justify-start">
                <div class="w-4/12">
                    <p><label for="numOfTestAttempt">Hozzárendelt csoportok: </label></p>
                </div>
                <div class="ml-6 w-full">
                    <div id="container">
                        <div class="lg:flex">
                            <div class="mb-2">
                                <div class="w-full ">
                                    <input type="text" wire:model.debounce.500ms="searchValue" id="emailInputField"
                                           placeholder="Minta Csoport" autocomplete="off"
                                            ...
                                            <div id="searchResults" wire:loading.remove
                                                ...
                                                @if(!empty($searchResults))
                                                    @foreach($searchResults as $index => $searchResult)
                                                        <button
                                                            class="px-2 py-1 font-semibold text-md hover:bg-blue-400 border-x-2 border-blue-800 bg-blue-50 w-full text-left"
        ...
        <div class="max-w w-full lg:px-8 md:px-6 sm:px-4 px-2 bg-slate-700 rounded-lgshadow-xl flex flex-col">
            @foreach ($tasks as $taskIndex => $task)
                <div
                    class="lg:pl-12 lg:py-12 lg:my-12 md:pl-9 md:py-9 md:my-9 sm:pl-6 sm:py-6 sm:my-6  pl-3 pr-2 py-3 my-3 max-w w-full bg-white rounded-lg  shadow-lg">
                    <div class="flex flex-col">
                        <div class="flex sm:flex-row flex-col mb-2">
                            <div>
                                <select name="task-types{{$taskIndex}}"
                                        wire:model="tasks.{{$taskIndex}}.type"
                            ...
                            <div class="flex mt-3">
                                @if($task['type'] != "")
                                    <button wire:click="Add_Question({{$taskIndex}})"
                                            class="flex items-center text-white bg-green-700 hover:bg-green-800 focus:outline-none focus:ring-4 focus:ring-green-300 font-medium rounded-full text-sm px-2.5  text-center ml-1.5 mb-2 dark:bg-green-600 dark:hover:bg-green-700 dark:focus:ring-green-800">
                                        <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24"
                                             stroke-width="2.0" stroke="white" class="w-4 h-4">
                                ...
                                @endif
                                <button wire:click="Remove_Task({{$taskIndex}})"
                                        class="text-white bg-red-700 hover:bg-red-800 focus:outline-none focus:ring-4 focus:ring-red-300 font-medium rounded-full text-sm px-3  text-center ml-1.5 mb-2 dark:bg-red-600 dark:hover:bg-red-700 dark:focus:ring-red-900">
                                    <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24"
                        ...
                        <div class="lg:px-12 md:px-8 px-4  py-6">
                            <div class="flex sm:flex-row flex-col">
                                <div class="w-full h-full">
                                    <input type="text"
                                           wire:model="tasks.{{ $taskIndex }}.questions.{{$questionIndex}}.text"
                                           placeholder="{{$questionIndex}}. Kérdés szövege"
                                ...
                                <div class="flex sm:my-0 my-2">
                                    @if($task['type'] != "TrueFalse")
                                        <button wire:click="Add_Answer({{$taskIndex}},{{$questionIndex}})"
                                    ...
                                    @endif
                                    <button wire:click="Remove_Question({{$taskIndex}},{{$questionIndex}})"
                            ...
                            @switch($task['type'])
                                @case('TrueFalse')
                                    <div class="flex flex-col md:flex-row md:items-center md:justify-center">
                                        @break
                                        @case('Sequence')
                                            <div wire:sortable="updateAnswerOrder">
                                                @break
                                                @default
                                                    <div>
                                                        @endswitch
                                                        @foreach($question['answers'] as $answerIndex => $answer)
                                                            @switch($task["type"])
                                                                @case("TrueFalse")
                                                                    <div
                                                                        ...
                                                                        <label>
                                                                            {{$answer["text"]}}
                                                                            <input type="radio"
                                                                                   name="Answer_{{$taskIndex}}_{{$questionIndex}}"
                                                                                   value="{{$answer['id']}}"
                                                                                   wire:model="tasks.{{ $taskIndex }}.questions.{{$questionIndex}}.right_answer_index">
                                                                        </label>
                                                                    </div>
                                                                    @break
                                                                @case("OneChoice")
                                                                    <div
                                                                        ...
                                                                        <div class="w-full select-none	">
                                                                            <label>
                                                                                <input type="radio"
                                                                                       name="Answer_{{$taskIndex}}_{{$questionIndex}}"
                                                                                       value="{{$answer['id']}}"
                                                                                       wire:model="tasks.{{ $taskIndex }}.questions.{{$questionIndex}}.right_answer_index">Ez
                                                                                a helyes
                                                                            </label>
                                                                            <input type="text"
                                                                                   wire:model="tasks.{{ $taskIndex }}.questions.{{$questionIndex}}.answers.{{$answerIndex}}.text"
                                                                                   placeholder="{{$answerIndex}}. Válasz szövege"
                                                                        ...
                                                                        <div class="grid place-items-center my-3">
                                                                            @if($answerIndex == $question['right_answer_index'])
                                                                                <input type="number"
                                                                                       wire:model="tasks.{{ $taskIndex }}.questions.{{$questionIndex}}.answers.{{$answerIndex}}.score"/>
                                                                            @endif
                                                                            <button
                                                                                wire:click="Remove_Answer({{$taskIndex}},{{$questionIndex}}, {{$answerIndex}})"
                                                                                class=" text-white bg-red-700 hover:bg-red-800 focus:outline-none focus:ring-4 focus:ring-red-300 font-medium rounded-full text-sm px-3 py-2.5  text-center ml-1.5 dark:bg-red-600 dark:hover:bg-red-700 dark:focus:ring-red-900">
                                                                        ...
                                                                        <div class="w-full">
                                                                            <label>
                                                                                <input type="checkbox"
                                                                                       name="Answer_{{$taskIndex}}_{{$questionIndex}}"
                                                                                       value="{{$answerIndex}}"
                                                                                       wire:model="tasks.{{ $taskIndex }}.questions.{{$questionIndex}}.answers.{{$answerIndex}}.solution">
                                                                            </label>
                                                                            <input type="text"
                                                                                   wire:model="tasks.{{ $taskIndex }}.questions.{{$questionIndex}}.answers.{{$answerIndex}}.text"
                                                                                   placeholder="{{$answerIndex}}. Válasz szövege"
                                                                ...
                                                                @case("Sequence")
                                                                    <div
                                                                        class="select-none sm:flex-row flex-col lg:ml-16 lg:px-8 lg:my-2 lg:py-4 ml-2 my-1 py-1 pl-6 flex items-center rounded border border-gray-200 bg-slate-100 hover:bg-slate-300"
