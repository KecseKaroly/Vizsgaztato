<div>
    <div id="container">
        <div class="lg:flex">
            <div class="mb-2">
                <div class="w-full ">
                    <input type="text" wire:model.debounce.500ms="searchValue" id="emailInputField" placeholder="mintamarton@email.com" autocomplete="off" class="font-semibold text-md focus:border-blue-800 focus:bg-blue-100 bg-blue-50 rounded-t-lg border-blue-300  lg:w-fit w-full" size="40"/>
                                ...
                                @if(!empty($searchResults))
                                    @foreach($searchResults as $index => $searchResult)
                                        <div class="px-2 py-1 font-semibold text-md hover:bg-blue-400 border-x-2 border-blue-800 bg-blue-50 w-full">
                                            <button wire:click="addToSelectedResults({{ $index }})">{{ $searchResult['email'] }}</button>
                                        </div>
                                    @endforeach
                                @else
            ...
            <div>
                <button wire:click="saveSelectedResults()" class="hover:bg-green-700 bg-green-500 border-2 border-green-900  text-white font-bold p-2 lg:ml-5"><i class="fa-solid fa-user-plus"></i> Meghívás</button>
            </div>
        </div>
    </div>
