...
<div class="mt-24 mb-24 select-none">
                ...
                <div class="my-6">
                    <div class="bg-slate-600 rounded-lg">
                        <div class=" flex w-full justify-between text-2xl py-2 px-2 rounded-md">
                            ...
                            <div>
                                <button data-modal-target="testGroupDetails" data-modal-toggle="testGroupDetails"
                                    data-group_id="{{ $group->id }}"
                                    data-test_id="{{ $test->id }}"
                                    data-enabled_from="{{  }}"
                                    data-enabled_until="{{ $ }}">Kezelés</button>
                            </div>
                            <div class="pr-6">
                                <button data-group_id="{{ $group->id }}" class="group_id hover:cursor-pointer hover:bg-slate-800  text-2xl md:w-10 md:h-10 w-6 h-6 rounded-full bg-slate-400  text-white">
                        ...
                        @foreach($groups_users as $group_user)
                            @if($group_user->group_id == $group->id)
                                ...
                                @if($user->id == $group_user->user_id)
                                    <div class="bg-slate-500 rounded-md flex w-11/12 justify-between text-xl font-semibold py-2 px-3 ml-16">
                                        <div class="flex divide-x-2">
                                        ...
                                        <div class="pr-6">
                                            <button data-user_id="{{ $user->id }}" data-group_id="{{ $group->id }}" class="user_id hover:cursor-pointer hover:bg-slate-800  text-2xl md:w-10 md:h-10 w-6 h-6 rounded-full bg-slate-400  text-white">
                                                <i class="fa-solid fa-angles-down" id="arrow_of_user{{ $user->id }}_{{ $group->id }}"></i>
                                            </button>
                                        </div>
                                    </div>
                                    <div id="attempts_of_user{{ $user->id }}_{{ $group->id }}" style="display: none;">
                                        @foreach ($testAttempts as $testAttempt )
                                            ...
                                            <div id="testAttempt#{{ $testAttempt->id }}" class="hover:bg-blue-400 bg-slate-400 rounded-md flex w-10/12 justify-between text-sm px-6 py-1 ml-32">
                                                <div class="mr-12">
                                                    <a href="{{ route('checkAttemptResult', [$test->id, $testAttempt->id]) }}">
                                                        <button class="bg-lime-200 p-1.5 border rounded-lg text-lime-900 hover:bg-lime-100">
                                                ...
                                                <div>
                                                    <form>
                                                        <button data-id="{{ $testAttempt->id }}" class="deleteTestAttempt rounded-full md:w-10 md:h-10 w-6 h-6 bg-red-600 hover:bg-red-800">
  ...
  <div id="testGroupDetails" tabindex="-1" class="fixed top-0 left-0 right-0 z-50 hidden w-full p-4 overflow-x-hidden overflow-y-auto md:inset-0 h-modal md:h-full">
      <div class="relative w-full h-full max-w-2xl md:h-auto">
          <div class="relative bg-white rounded-lg shadow dark:bg-gray-700">
              <div class="flex items-start justify-between p-4 border-b rounded-t dark:border-gray-600">
                  ...
                  <button data-modal-hide="testGroupDetails" type="button" class="text-gray-400 bg-transparent hover:bg-red-500 hover:text-white rounded-lg p-1.5">
              ...
              <div class="flex items-center p-6 justify-evenly border-t-2">
                  <button data-modal-hide="testGroupDetails" type="button" class="border rounded bg-green-500 hover:bg-green-700 text-white p-2">Mentés</button>
                  <button data-modal-hide="testGroupDetails" type="button" class="border rounded bg-transparent hover:bg-stone-300 text-black p-2 ">Mégsem</button>
              </div>
          </div>
      </div>
  </div>
