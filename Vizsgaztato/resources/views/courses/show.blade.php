...
@section('content')
    <div class="mt-4 pb-6">
        ...
        <div class="flex flex-col max-w-full mx-auto rounded-xl overflow-hidden w-11/12">
            ...
            @endif
            <div class="absoulte">
                <div class="grid grid-cols-4 bg-amber-100 relative top-10 right-0 left-0 md:mx-32 mx-1 border-4 border-amber-800 rounded-full
                           text-center content-center divide-x-4 divide-amber-800 divide-dashed">
                    <a href="{{route('courses.members', $course)}}" class="md:py-8 py-4 block hover:bg-amber-200 hover:rounded-l-full">
                    ...
                    <a href="{{route('courses.modules', $course)}}" class="md:py-8 py-4 block hover:bg-amber-200">
                        <div class="md:text-xl font-semibold">Modulok</div>
                        <div class="md:text-xs text-2xs">{{$course->modules->count()}} db</div>
                    </a>
                    <a href="{{route('quizzes.index', $course)}}" class="md:py-8 py-4 block hover:bg-amber-200">
                        <div class="md:text-xl font-semibold">Kvízek</div>
                        <div class="md:text-xs text-2xs">{{$course->quizzes->count()}} db</div>
                    </a>
                    <a href="{{route('test.index', $course)}}" class="md:py-8 py-4 block hover:bg-amber-200 hover:rounded-r-full">
                        <div class="md:text-xl font-semibold">Tesztek</div>
                        <div class="md:text-xs text-2xs">{{$course->exams->count()}} db</div>
                    </a>
                </div>
                <div
                    class="bg-yellow-50 rounded-xl mt-4 h-fit">
                    <div class="w-full md:flex  md:divide-x-8
                    md:divide-amber-800 md:divide-double">
                        <div class="md:w-1/2 w-full">
                            <div
                                class="px-6 py-8 mb-4 text-2xl underline font-semibold bg-amber-400 border-4 md:border-r-0 border-amber-800">
                                A kurzus célja:
                            </div>
                        ...
                        <div class="md:w-1/2 w-full md:mt-0 mt-6">
                            <div
                                @class([
                                         "md:flex justify-between px-6 mb-4 text-2xl bg-amber-400 border-4 md:border-l-0 border-amber-800 items-center",
                                ...
                                <div class="w-fit md:mt-0 mt-2">
                                    @can('create', [App\Models\Module::class, $course])
                                        <button type="button"
                                                class="hover:bg-green-700 bg-green-500 border-2 border-amber-100  text-white font-bold p-2 rounded-lg text-sm">
                            ...
                            <table class="table-auto w-11/12 mx-auto  border-separate border border border-black border-spacing-y-4 bg-yellow-500">
                                @if( count($course->modules) )
                                    <tr class="hover:cursor-pointer bg-yellow-500 w-10/12 mx-auto mt-2 mb-2 text-amber-100 px-3 py-2 text-center">
                                ...
                                @forelse($course->modules->take(4) as $module)
                                    <tr class="hover:cursor-pointer bg-yellow-600 w-10/12 mx-auto mt-2 mb-2 text-amber-100 px-3 py-2 text-center"
